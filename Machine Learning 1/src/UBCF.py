...
class CollaborativeFilteringRecommender(object):
    ...
    def present_user(self, user_profile, user_ratings):
        print "User's favourite genre:", self.dataset_handler.feature_index2genre(np.argmax(user_profile[0]))
...
users_ratings_data = dataset_handler.load_users_ratings()
users_ratings_data
...
pickle.dump(users_ratings_data, open(completeName1, 'wb'))
...
users_ratings = pickle.load(open('../models/UBCF.sav', 'rb'))
...
#delete for user ID being considered
del users_ratings[1]
users_ratings
